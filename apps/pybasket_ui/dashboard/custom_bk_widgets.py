...
def meta_button(metadata_dict, host_layout, content_type='metadata'):
    ...
    if content_type == 'NA':
        resources = literal_eval(metadata_dict)['resources']
        if 'OGC:WMS' in resources:
            content = f"<iframe src='https://bokeh.epinux.com/GISPY?url={resources['OGC:WMS'][0]}' width='1225' height='725' frameborder=0 scrolling=no></iframe>"
        else:
            content = f"Plotting routine not implemented for this featureType: {resources}, {resources.keys()}"
        icon_name = "line-chart"
