...
import uuid
import base64
from nc_transform import get_data
...
from utility import ModelURL, get_labels
...
templates = Jinja2Templates(directory="tsp_plot/templates")
...
if os.getenv("TSPLOT_DOWNLOAD") is not None:
...
def data_download(event):
    ...
    with pn.param.set_values(bokeh_pane, loading=True):
        ...
        if data.feature_type in ["TimeSeriesProfile", "Profile"]:
            ...
            index_range = (
                np.array([p.x_range.start, p.x_range.end], dtype="i8")
                .view("datetime64[ms]")
                .tolist()
            )
        ...
        if export_resampling.active and resampling.value != "--":
            df_export = (
                df[[variables[x] for x in export_variables.active]][
                    df.index[start] : df.index[end - 1]
                ]
                .resample(resampling.value)
                .mean()
                .interpolate(method="linear")
            )
        else:
            df_export = df[[variables[x] for x in export_variables.active]][
                df.index[start] : df.index[end - 1]
            ]
...
def resampler(attr, old, new):
    ...
    with pn.param.set_values(bokeh_pane, loading=True):
        ...
        if new == 0 or new == "--":
            ...
            new_freq = pandas_frequency_offsets[new]
            data = df.resample(new_freq).mean().interpolate(method="linear").copy()
...
def tsp_handler(value, old, new):
    ...
    with pn.param.set_values(bokeh_pane, loading=True):
        ...
        tooltips = [(df.index.name, "@" + df.index.name), (xlabel, var_label)]
...
def ts_handler(value, old, new):
    with pn.param.set_values(bokeh_pane, loading=True):
        ...
        var_label = "@{" + str(new + "}")
        tooltips = [("Time", "@tooltip"), (ylabel, var_label)]
        hover.tooltips = tooltips
...
def get_y_label(selected_variable):
    ...
    try:
        ...
        var_tooltip_label = str(
            df.variable_metadata[selected_variable]["standard_name"]
        )
    try:
        units = list(
            {"unit", "units"}.intersection(df.variable_metadata[selected_variable])
        )[0]
        y_axis_label = " ".join(
            [
                var_tooltip_label,
                "[",
                df.variable_metadata[selected_variable][units],
                "]",
            ]
        )
    except IndexError:
...
def increase_font(event):
    ...
    major_label_text_font_size = int(
        p.xaxis.major_label_text_font_size.replace("pt", "")
    )
...
def build_metadata_widget(data):
    ...
    metadata_button = Button(
        icon=FontAwesomeIcon(icon_name="info", size=2),
        label="",
        height=50,
        width=50,
    )  # , width_policy='fixed'
...
if nc_url and valid_url:
    try:
        data = get_data(nc_url)
        metadata_layout, metadata_button = build_metadata_widget(data)
            ...
            slider_wrapper = layout(
                [
                    [
                        ...
                        column(
                            Spacer(width=10, height=20), row(left_btn, right_btn, par)
                        ),
                    ],
                ]
            )
        ...
        increase_fontsize = Button(
            icon=FontAwesomeIcon(icon_name="plus", size=1),
            height=30,
            width=30,
            label="",
        )
        decrease_fontsize = Button(
            icon=FontAwesomeIcon(icon_name="minus", size=1),
            height=30,
            width=30,
            label="",
        )
        ...
        # curdoc_element.add_root(bk_pane)
else:
    raw_data = Div(
        text=f"""<br><br><b>Can't load dataset from URL: {nc_url}</b> <br> Usage example: <br> http://endpoint/tsp_plot?url=http://opendap_url"""
    )
    url_data = ""
    if not valid_url:
        url_data = Div(text=f"""<br><br>{nc_url} is not a valid URL""")
    bokeh_pane = pn.pane.Bokeh(column(raw_data, url_data)).servable()
    # curdoc_element.add_root(bk_pane)


# bokeh_pane = pn.pane.Bokeh(bokeh_pane).servable()
