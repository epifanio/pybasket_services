...
def handle_interaction(**kwargs):
    ...
    if kwargs.get("type") == "mousemove":
        ...
        lon = "%.2f" % round(lon, 4)
        lonlat_label.value = (
            f'<p style="font-size:100%;"><b>Latitude: {lat}, Longitude: {lon}</b></p>'
        )
    if kwargs.get("type") == "click":
        lat, lon = kwargs.get("coordinates")
        lat = "%.2f" % round(lat, 4)
        lon = "%.2f" % round(lon, 4)
        outclick_label.value = (
            f'<p style="font-size:100%;"><b>Latitude: {lat}, Longitude: {lon}</b></p>'
        )
    if kwargs.get("type") == "mousedown":
        ...
        for i in ztwms_controls:
            ...
            print(
                "elevation: ", str(i.wms_control.children[1].children[4].value).strip()
            )
...
def reload_view(value, old, new):
    ...
    ztwms_controls_new = [
        ztwms_control(
            wms_url=v, crs=int(crs_selector.value.split(":")[1]), m=m, ipygis_key=str(i)
        )
    ...
    control_box = VBox(
        [i.wms_control for i in ztwms_controls[::-1]],
    ...
    wrap_controller_menu = IPyWidget(widget=control_box, width_policy="fit")
    wrap_map_container = IPyWidget(widget=m, sizing_mode="scale_both", height=900)

    wrap_outclick_label = IPyWidget(
        widget=outclick_label, width_policy="fit", height=40
    )
    wrap_lonlat_label = IPyWidget(widget=lonlat_label, width_policy="fit", height=40)

    metadata_button = bkButton(
        icon=FontAwesomeIcon(icon_name="map-marker", size=2),
        label="",
        height=50,
        width=50,
        width_policy="fixed",
    ...
    menu_control = column(
        [metadata_button, crs_selector, wrap_lonlat_label, wrap_outclick_label],
        height_policy="fit",
    )
    wrap_controller_menu_scroll = column(
        [wrap_controller_menu], height=500, height_policy="fit"
    )
    layout = row(
        [
            menu_control,
...
crs_selector = Select(
    title="CRS", value=common_crs[0], options=common_crs, css_classes=["custom_select"]
)
...
def initiate_map(wms_urls, init_prj):  # , top_box
    ...
    ztwms_controls = [
        ztwms_control(wms_url=v, crs=init_prj, m=m, ipygis_key=str(i))
        for i, v in enumerate(wms_urls)
    ]
...
m, ztwms_controls, controller_menu = initiate_map(
    wms_urls,
    init_prj,
)
